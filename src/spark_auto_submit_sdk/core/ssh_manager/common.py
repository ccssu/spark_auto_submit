...
class BaseNode:
    ...
    def connect(self):
        raise NotImplementedError(
            "connect() method must be implemented in the subclass"
        )

    def execute_command(self, command):
        raise NotImplementedError(
            "execute_command() method must be implemented in the subclass"
        )

    def transfer_data(self, source_path, destination_path, callback=None):
        raise NotImplementedError(
            "transfer_data() method must be implemented in the subclass"
        )
    ...
    def _upload_file(self, sftp_client, local_file, remote_directory):
        ...
        with tqdm(
            ...
            def callback(bytes_so_far, bytes_to_be_transferred):
                ...
                progress_bar.update(convert_bytes_to_mb(bytes_so_far) - progress_bar.n)
